...
namespace DecaBlog.Controllers
{
    ...
    public class SquadController : ControllerBase
    {
        ...
        [HttpPut("update-squad/{squadId}")]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> UpdateSquad(string squadId, [FromBody] SquadToUpdateDto model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ResponseHelper.BuildResponse<object>(false, "Validation errors", ModelState, null));
            }

            var result = await _squadService.UpdateSquad(squadId, model);
            if (result.Item1 == false)
            {
                return BadRequest(ResponseHelper.BuildResponse<object>(false, result.Item2, ResponseHelper.NoErrors, null));
            }

            return Ok(ResponseHelper.BuildResponse<string>(true, " Updated Successfully", ResponseHelper.NoErrors, result.Item2));
        }
    }
}
