...
namespace DecaBlog.Controllers
{
    ...
    public class StackController : ControllerBase
    {
        ...
        [HttpPatch("update-stack/{stackId}")]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> UpdateStack([FromRoute] string stackId, [FromBody] StackToUpdateDto model)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ResponseHelper.BuildResponse<StackMinInfoToReturnDto>(false, "Invalid input. Please try again.", ModelState, null));
            }

            var data = await _stackService.UpdateStack(stackId, model);
            if (data == null)
            {
                ModelState.AddModelError("Stack", "Stack details update failed. Please try again.");
                return BadRequest(ResponseHelper.BuildResponse<StackMinInfoToReturnDto>(false, "Stack details update failed. Please try again.", ModelState, null));
            }

            return Ok(ResponseHelper.BuildResponse(true, "Update successful", ResponseHelper.NoErrors, data));
        }
    }
}
