...
namespace DecaBlog.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetUserById(string userId)
        {
            ...
            var okRes = ResponseHelper.BuildResponse(true, "User information", ResponseHelper.NoErrors, userToreturn);
            return Ok(okRes);
        }

        [HttpGet("search")]
        public async Task<IActionResult> SearchUsers([FromQuery] string name, [FromQuery] int pageNumber, [FromQuery] int perPage)
        {
            if (perPage == 0) perPage = 10;
            var result = await _userService.SearchUsersByName(name, pageNumber, perPage);
            if (result.MetaData == null)
            {
                ModelState.AddModelError("NotFound", "No record of users found");
                var response = ResponseHelper.BuildResponse<List<UserMinInfoToReturnDto>>(false, "No results found!", ModelState, null);
        ...
        [HttpGet("search-invitees")]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> SearchInvitee(string name, int pageNumber = 1, int perPage = 20) =>
            Ok(ResponseHelper.BuildResponse(true, "success", ResponseHelper.NoErrors,
                await _userService.SearchInviteeByName(name, pageNumber, perPage)));

        [HttpGet("get-invitee/{id}")]
        public async Task<IActionResult> GetInviteeById([FromRoute] string id)
        {
            var invitee = await _userService.GetInviteeById(id);
            if (invitee == null)
            {
                var res = ResponseHelper.BuildResponse<object>(false, "Invitee not foud", ResponseHelper.NoErrors, null);
                return NotFound(res);
            }

            var resToReturn = ResponseHelper.BuildResponse(true, "invitee", ResponseHelper.NoErrors, invitee);
