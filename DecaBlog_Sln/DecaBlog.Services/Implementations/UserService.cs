...
namespace DecaBlog.Services.Implementations
{
    public class UserService : IUserService
    {
        ...
        {
            var currentUserId = _httpContextAccessor.HttpContext?.User.GetLoggedInUserId<string>();
        ...
        public async Task<RegisterResponseDto> AddDecadev(UserToRegisterDto model)
        {
            ...
            user.InvitationToken = await _userMgr.GenerateUserTokenAsync(user, "Invitation", "DecadevInvitation");
        ...
        public async Task<(bool, string)> ApproveInvitee(string inviteId)
        {
            ...
            await _inviteeRepository.DeleteInvitee(invitee);
            return (true, "invitation approved and confirmed");
        }

        public async Task<PaginatedListDto<UserMinInfoToReturnDto>> SearchUsersByName(string name, int pageNumber, int perPage)
        {
            var usersToReturn = new List<UserMinInfoToReturnDto>();
            name = (name == null) ? name : name.ToLower();
            IEnumerable<User> users = default;
            if (name == null)
            {
                users = _userMgr.Users
                .Include(x => x.UserStacks).ThenInclude(u => u.Stack)
                .Include(x => x.UserSquads).ThenInclude(u => u.Squad)
            ...
            else
            {
                users = _userMgr.Users
                    .Include(x => x.UserStacks).ThenInclude(u => u.Stack)
                    .Include(x => x.UserSquads).ThenInclude(u => u.Squad)
                    .Search(x => x.FirstName.ToLower(), x => x.LastName.ToLower())
                    .Containing(name.Split(' '))
                    .ToRanked().OrderByDescending(r => r.Hits)
                    .Select(x => x.Item);
            }
        ...
        public async Task<PaginatedListDto<InviteeSearchToReturnDto>> SearchInviteeByName(string author, int pageNumber, int perPage)
        {
            var searchParams = author == null ? new string[] { } : author.ToLower().Split(" ", StringSplitOptions.TrimEntries);
            var inviteeByName = _inviteeRepository.SearchInviteeByName(searchParams);
            var inviteeToReturn = new List<InviteeSearchToReturnDto>();
            var pagedList = PagedList<Invitee>.Paginate(inviteeByName, pageNumber, perPage);
            foreach (var invitee in pagedList.Data)
            {
                inviteeToReturn.Add(new InviteeSearchToReturnDto
                {
                    Id = invitee.InviteeId,
                    FirstName = invitee.FirstName,
                    LastName = invitee.LastName,
                    Email = invitee.Email,
                    Stack = (await _stackRepository.GetStackById(invitee.StackId))?.Name,
                    Squad = (await _squadRepository.GetSquad(invitee.SquadId))?.Name
                });
            }
            var result = new PaginatedListDto<InviteeSearchToReturnDto>
            {
                MetaData = pagedList.MetaData,
                Data = inviteeToReturn
            };
        ...
        public async Task<InviteeSearchToReturnDto> GetInviteeById(string inviteeId)
        {
            var invitee = await _inviteeRepository.GetInviteeById(inviteeId);
            if (invitee == null)
                return null;

            var inviteeToReturn = new InviteeSearchToReturnDto
            {
                Id = invitee.InviteeId,
                FirstName = invitee.FirstName,
                LastName = invitee.LastName,
