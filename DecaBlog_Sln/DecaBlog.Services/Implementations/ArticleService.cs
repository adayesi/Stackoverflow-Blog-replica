...
namespace DecaBlog.Services.Implementations
{
    public class ArticleService : IArticleService
    {
        ...
        public ArticleService(IArticleTopicRepository articleTopicRepository, IArticleRepository articleRepository, IMapper mapper, 
            IHttpContextAccessor httpContextAccessor, UserManager<User> userManager,IArticleSearchRepository articleSearchRepository,
        ...
        public Task<PaginatedListDto<AllArticlesToReturnDto>> GetAllArticleTopicsWithPublishedArticles(ArticleRequestParameter parameters)
        {
            var allArticlesReturns = _articleRepository.GetAllArticleTopicsWithPublishedArticles();
            List<AllArticlesToReturnDto> tableMapping = new List<AllArticlesToReturnDto>();
            var returnDto = _mapper.Map<List<AllArticlesToReturnDto>>(allArticlesReturns);
            //iterate over the keywords to create tags for the article
            foreach (var item in returnDto)
        ...
        public async Task<bool> AddLikeToArticleTopic(User user, string topicId)
        {
            ...
            if (!checker)
            {
                ...
                await _notifications.AddLikeNotification(user, topicId);
                return true;
            }
