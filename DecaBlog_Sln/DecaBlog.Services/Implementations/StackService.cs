...
namespace DecaBlog.Services.Implementations
{
    public class StackService : IStackService
    {
        ...
        public async Task<StackMinInfoToReturnDto> UpdateStack(string id, StackToUpdateDto model)
        {
            var stack = await _stackRepository.GetStackById(id);
            if (stack == null)
            {
            ...
            _mapper.Map<Stack>(model);
            var isUpdated = await _stackRepository.UpdateStack(stack);
            if (!isUpdated)
            {
                return null;
            }
