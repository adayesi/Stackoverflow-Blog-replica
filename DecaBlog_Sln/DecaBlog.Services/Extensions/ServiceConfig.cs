...
namespace DecaBlog.Services.Extensions
{
    public static class ServiceConfig
    {
        public static IServiceCollection ConfigureCoreServices(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.Configure<FirebaseClientSettings>(configuration.GetSection(nameof(FirebaseClientSettings)));
            services.AddHttpClient<IFirebaseClient, FirebaseClient>(opt => {
                var firebaseSettings = new FirebaseClientSettings();
                configuration.GetSection(nameof(FirebaseClientSettings)).Bind(firebaseSettings);
                opt.BaseAddress = new Uri(firebaseSettings.Url);
            });
